...
object Day7 {
    def main(args: Array[String]): Unit = {
        val string = """czlmv (78)
                       ...
                       |tvhftq (35)""".stripMargin

        val v = Seq(string.split("\n")).map(array=>{
            val regex = "^([a-z]+) \\(([0-9]+)\\)(?: -> (.*)?)?$".r.pattern
            array.map({ e=>
                val m = regex.matcher(e)
                m.find()
                val name = m.group(1)
                if(m.groupCount() == 3 && m.group(3) != null) {
                    (name, m.group(3).split(", ").toList)
                } else {
                    (name, List())
                }
            })
        }).map(array=>{
            array.find(e=>array.forall(!_._2.contains(e._1))).get._1
        }).head
        ...
        val v2 = Seq(string.split("\n")).map(array=>{
            ...
            array.map({ e=>
                ...
                val name = m.group(1)
                if(m.groupCount() == 3 && m.group(3) != null) {
                    (name, m.group(2).toInt, m.group(3).split(", ").toList)
                } else {
                    (name, m.group(2).toInt, List())
                }
            })
        }).map(array=>{
            val parents: Map[String, String] = array.flatMap{ case (parent, _, nodes) => nodes.map(child => child -> parent) }.toMap
            val children: Map[String, (Int, List[String])] = array.map{ case (node, weight, nodes) => node -> (weight, nodes) }.toMap

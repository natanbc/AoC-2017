...
object Day8 {
    def main(args: Array[String]): Unit = {
        val string = """gug dec 188 if zpw >= 8
                       ...
                       |g dec -541 if u == -2614""".stripMargin

        val v = Seq(string.split("\n")).map(array=>{
            array.map({instruction=>{
                val m = Pattern.compile("(\\S+) (inc|dec) (-?\\d+) if (\\S+) (\\S+) (-?\\d+)").matcher(instruction)
                m.find()
                (m.group(1), if(m.group(2) == "inc") m.group(3).toInt else -m.group(3).toInt, m.group(4), m.group(5), m.group(6).toInt)
            }})
        }).map(array=>{
            val registers = new mutable.HashMap[String, Int]()
            array.foreach(instruction=>{
                val currentValueInFilterRegister = registers.getOrElse(instruction._3, 0)
                val value = instruction._5
                val shouldApply: Boolean = instruction._4 match {
                    case "==" => currentValueInFilterRegister == value
                    case "!=" => currentValueInFilterRegister != value
                    case ">" => currentValueInFilterRegister > value
                    case ">=" => currentValueInFilterRegister >= value
                    case "<" => currentValueInFilterRegister < value
                    case "<=" => currentValueInFilterRegister <= value
                    case other => throw new AssertionError(s"Unknown filter $other")
                }
                if(shouldApply) {
                    registers.put(instruction._1, instruction._2 + registers.getOrElse(instruction._1, 0))
                }
            })
        ...
        val v2 = Seq(string.split("\n")).map(array=>{
            ...
            array.foreach(instruction=>{
                ...
                if(shouldApply) {
                    val old = registers.getOrElse(instruction._1, 0)
                    val n = instruction._2 + old
                    if(old > highest) highest = old
                    if(n > highest) highest = n
                    registers.put(instruction._1, n)
                }
            })
            highest
        }).head

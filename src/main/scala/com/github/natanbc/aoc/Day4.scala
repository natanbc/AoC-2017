...
object Day4 {
    def main(args: Array[String]): Unit = {
        ...
        val v = string.split("\n").map(_.split("\\s")).count(passphrase=>passphrase.forall(word=>passphrase.count(_ == word) == 1))

        println(v)

        val v2 = string.split("\n").map(_.split("\\s")).count(passphrase=>passphrase.map(_.toCharArray.groupBy(identity).mapValues(_.length)).groupBy(identity).values.forall(_.length == 1))
